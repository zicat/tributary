...
public class BeatsParser extends ByteToMessageDecoder {
        ...
        if (!hasEnoughBytes(in)) {
            if (decodingCompressedBuffer) {
                throw new InvalidFrameProtocolException(
                        "Insufficient bytes in compressed content to decode: " + currentState);
            }
            return;
        }
            ...
            case READ_JSON:
                {
                    ...
                    if (batch.isComplete()) {
                        logger.trace(
                                "Sending batch size: {}, windowSize: {}, seq:{}",
                                batch.size(),
                                batch.getBatchSize(),
                                sequence);
                        out.add(batch);
    ...
    private void inflateCompressedFrame(
            final ChannelHandlerContext ctx, final ByteBuf in, final CheckedConsumer<ByteBuf> fn)
            throws IOException {
    ...
    private void decompressImpl(final ByteBuf in, final ByteBuf out) throws IOException {
        ...
        try (ByteBufOutputStream buffOutput = new ByteBufOutputStream(out);
                InflaterOutputStream inflaterStream =
                        new InflaterOutputStream(buffOutput, inflater)) {
    ...
    private void transition(States nextState, int requiredBytes) {
        logger.trace(
                "Transition, from: {}, to: {}, requiring {} bytes",
                currentState,
                nextState,
                requiredBytes);
