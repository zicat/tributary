...
import org.zicat.tributary.source.netty.kafka.inner.ApiVersionsResponseData.ApiVersionsResponseKey;
import org.zicat.tributary.source.netty.kafka.inner.ApiVersionsResponseData.ApiVersionsResponseKeyCollection;
...
public class ApiVersionsRequest extends AbstractRequest {
    ...
    public static class Builder extends AbstractRequest.Builder<ApiVersionsRequest> {
        ...
        private static final ApiVersionsRequestData DATA =
    ...
    private static final Pattern SOFTWARE_NAME_VERSION_PATTERN =
    ...
    public ApiVersionsRequest(
            ApiVersionsRequestData data, short version, Short unsupportedRequestVersion) {
        super(ApiKeys.API_VERSIONS, version);
        this.data = data;

        // Unlike other request types, the broker handles ApiVersion requests with higher versions
        // than
        // supported. It does so by treating the request as if it were v0 and returns a response
        // using
        // the v0 response schema. The reason for this is that the client does not yet know what
        // versions
        // a broker supports when this request is sent, so instead of assuming the lowest supported
        // version,
    ...
    public boolean isValid() {
        if (version() >= 3) {
            return SOFTWARE_NAME_VERSION_PATTERN.matcher(data.clientSoftwareName()).matches()
                    && SOFTWARE_NAME_VERSION_PATTERN
                            .matcher(data.clientSoftwareVersion())
                            .matches();
        } else {
            return true;
        }
    ...
    public ApiVersionsResponse getErrorResponse(int throttleTimeMs, Throwable e) {
        ApiVersionsResponseData data =
                new ApiVersionsResponseData().setErrorCode(Errors.forException(e).code());
