...
import org.zicat.tributary.source.netty.handler.kafka.MetadataRequestData.MetadataRequestTopic;
...
public class MetadataRequest extends AbstractRequest {
    ...
    public static class Builder extends AbstractRequest.Builder<MetadataRequest> {
        private static final MetadataRequestData ALL_TOPICS_REQUEST_DATA =
                new MetadataRequestData().setTopics(null).setAllowAutoTopicCreation(true);
        ...
        public Builder(
            ...
            if (topics == null) data.setTopics(null);
                ...
                topics.forEach(
                        topic -> data.topics().add(new MetadataRequestTopic().setName(topic)));
            }
        ...
        public static Builder allTopics() {
            // This never causes auto-creation, but we set the boolean to true because that is the
            // default value when
            // deserializing V2 and older. This way, the value is consistent after serialization and
            // deserialization.
    ...
    public boolean isAllTopics() {
        return (data.topics() == null)
                || (data.topics().isEmpty()
                        && version == 0); // In version 0, an empty topic list indicates
    ...
    public static List<MetadataRequestTopic> convertToMetadataRequestTopic(
            final Collection<String> topics) {
        return topics.stream()
                .map(topic -> new MetadataRequestTopic().setName(topic))
                .collect(Collectors.toList());
    }
