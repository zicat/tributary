...
package org.zicat.tributary.source.netty.kafka;
...
import org.zicat.tributary.source.netty.kafka.inner.ProduceResponse.PartitionResponse;
...
import static org.zicat.tributary.source.netty.kafka.inner.AbstractRequest.parseRequest;
import static org.zicat.tributary.source.netty.kafka.inner.ApiVersionsResponse.DEFAULT_API_VERSIONS_RESPONSE;
...
public abstract class KafkaMessageDecoder extends SimpleChannelInboundHandler<byte[]>
    ...
    private static final String HEADER_KEY_AUTH_USER = "auth_user";
    private static final KafkaRecordSerDeSer SER_DE_SER = new KafkaRecordSerDeSer();
    ...
    protected void channelRead0(ChannelHandlerContext ctx, byte[] msg) throws Exception {
        ...
        final AbstractRequest request = parseRequest(header.apiKey(), header.apiVersion(), struct);
    ...
    private void handleProduceRequest(
        ...
        for (Entry<TopicPartition, MemoryRecords> entry : allRecords.entrySet()) {
            ...
            final List<Header> extraHeaders = extraHeaders(receivedTs, ctx);
            final MemoryRecords records = entry.getValue();
            final ByteBuffer byteBuffer = SER_DE_SER.toByteBuffer(tp, extraHeaders, records);
            final int channelPartition = tp.partition() % channel.partition();
            try {
                channel.append(channelPartition, byteBuffer);
            } catch (Throwable e) {
     ...
     * @return header array
     */
    private List<Header> extraHeaders(int receivedTs, ChannelHandlerContext ctx) {
        final List<Header> result = new ArrayList<>();
        result.add(new RecordHeader(HEADER_KEY_REC_TS, toBytes(receivedTs)));
        final String user = ctx.channel().attr(KEY_AUTHENTICATED_USER).get();
        if (user != null) {
            result.add(
                    new RecordHeader(HEADER_KEY_AUTH_USER, user.getBytes(StandardCharsets.UTF_8)));
        }
        return result;
    }
