...
import org.zicat.tributary.source.netty.handler.kafka.MetadataResponseData.MetadataResponseBroker;
import org.zicat.tributary.source.netty.handler.kafka.MetadataResponseData.MetadataResponsePartition;
import org.zicat.tributary.source.netty.handler.kafka.MetadataResponseData.MetadataResponseTopic;
...
public class MetadataResponse extends AbstractResponse {
    ...
    public Cluster cluster() {
        ...
        return new Cluster(
                ...
                topicsByError(Errors.INVALID_TOPIC_EXCEPTION),
                internalTopics,
                controller());
    }

    public static PartitionInfo toPartitionInfo(
            PartitionMetadata metadata, Map<Integer, Node> nodesById) {
    ...
    private static Node[] convertToNodeArray(
            List<Integer> replicaIds, Map<Integer, Node> nodesById) {
        return replicaIds.stream()
                .map(
                        replicaId -> {
                            Node node = nodesById.get(replicaId);
                            if (node == null) return new Node(replicaId, "", -1);
                            return node;
                        })
                .toArray(Node[]::new);
    }
    ...
    public Optional<Integer> topicAuthorizedOperations(String topicName) {
        ...
        else return Optional.of(topic.topicAuthorizedOperations());
    }
    ...
    private Holder holder() {
        if (holder == null) {
            synchronized (data) {
                if (holder == null) holder = new Holder(data);
            }
        }
        return holder;
    }
    ...
    /**
     * Check whether the leader epochs returned from the response can be relied on for epoch
     * validation in Fetch, ListOffsets, and OffsetsForLeaderEpoch requests. If not, then the client
     * will not retain the leader epochs and hence will not forward them in requests.
     *
    ...
    public static class PartitionMetadata {
        ...
        public String toString() {
            return "PartitionMetadata("
                    ...
                    + leaderEpoch
                    + ", replicas="
                    + Utils.join(replicaIds, ",")
                    + ", isr="
                    + Utils.join(inSyncReplicaIds, ",")
                    + ", offlineReplicas="
                    + Utils.join(offlineReplicaIds, ",")
                    + ')';
        }
    }
    ...
    private static class Holder {
        ...
        private Map<Integer, Node> createBrokers(MetadataResponseData data) {
            return data.brokers().valuesList().stream()
                    .map(b -> new Node(b.nodeId(), b.host(), b.port(), b.rack()))
            ...
            for (MetadataResponseTopic topicMetadata : data.topics()) {
                ...
                for (MetadataResponsePartition partitionMetadata : topicMetadata.partitions()) {
                    ...
                    Optional<Integer> leaderIdOpt =
                            leaderId < 0 ? Optional.empty() : Optional.of(leaderId);
    ...
    public static MetadataResponse prepareResponse(
            int throttleTimeMs,
                        ...
                        metadataResponseTopic
                                ...
                                .add(
                                        new MetadataResponsePartition()
                                                ...
                                                .setLeaderId(
                                                        partitionMetadata.leaderId.orElse(
                                                                NO_LEADER_ID))
                                                .setLeaderEpoch(
                                                        partitionMetadata.leaderEpoch.orElse(
                                                                RecordBatch
    ...
    public static MetadataResponse prepareResponse(
        ...
        return prepareResponse(
                DEFAULT_THROTTLE_TIME, brokers, clusterId, controllerId, topicMetadata);
    }
