...
package org.zicat.tributary.source.test.netty.kafka;
...
import org.zicat.tributary.source.netty.kafka.KafkaMessageDecoder;
import org.zicat.tributary.source.netty.kafka.KafkaPipelineInitializationFactory;
import org.zicat.tributary.source.netty.kafka.inner.*;
import org.zicat.tributary.source.netty.kafka.inner.MetadataRequestData.MetadataRequestTopic;
import org.zicat.tributary.source.netty.kafka.inner.MetadataResponse.PartitionMetadata;
import org.zicat.tributary.source.netty.kafka.inner.MetadataResponse.TopicMetadata;
...
import static org.zicat.tributary.source.netty.kafka.KafkaPipelineInitialization.*;
import static org.zicat.tributary.source.netty.kafka.inner.ApiKeys.*;
...
public class KafkaPipelineInitializationFactoryTest {
    ...
    private void test(Channel channel, ReadableConfig config) throws Exception {
        try (DefaultNettySource source = createSource(config, channel, port1)) {
            ...
            final TopicMetadata topicMetadata = metaResponse.topicMetadata().iterator().next();
            ...
            final KafkaTopicPartitionRecords tpRecords = DE_SER.fromBytes(resultSet.next());
            Assert.assertEquals(tp.topic(), new String(tpRecords.getTopic()));
            final MutableRecordBatch batch = tpRecords.getMemoryRecords().batchIterator().next();
            final Record record = batch.iterator().next();
            Assert.assertEquals("k1", BytesUtils.toString(record.key()));
            Assert.assertEquals("v1", BytesUtils.toString(record.value()));
            Assert.assertEquals(1, record.headers().length);
            Assert.assertEquals("h1", record.headers()[0].key());
            Assert.assertEquals("hv1", new String(record.headers()[0].value()));
            ...
            try (DefaultNettySource source2 = createSource(config, channel, port2)) {
                ...
                final TopicMetadata topicMeta2 = metaResponse2.topicMetadata().iterator().next();
                Assert.assertEquals(partitions, topicMeta2.partitionMetadata().size());
                for (PartitionMetadata partitionMetadata : topicMeta2.partitionMetadata()) {
                    Assert.assertTrue(partitionMetadata.leaderEpoch.isPresent());
                    Assert.assertEquals(0, partitionMetadata.leaderEpoch.get().intValue());
                    Assert.assertTrue(partitionMetadata.leaderId.isPresent());
                }
                ...
                new MetadataRequestData()
                        .setTopics(
                                Collections.singletonList(
                                        new MetadataRequestTopic().setName(tp.topic()))),
