...
public class ProduceRequest extends AbstractRequest {
    ...
    private static final Schema TOPIC_PRODUCE_DATA_V0 =
            new Schema(
                    ...
                    new Field(
                            ...
                            new ArrayOf(
                                    new Schema(
                                            ...
                                            new Field(RECORD_SET_KEY_NAME, Type.RECORDS)))));
    ...
    private static final Schema PRODUCE_REQUEST_V0 =
            new Schema(
                    ...
                    new Field(TIMEOUT_KEY_NAME, Type.INT32, "The time to await a response in ms."),
                    new Field(TOPIC_DATA_KEY_NAME, new ArrayOf(TOPIC_PRODUCE_DATA_V0)));

    /**
     * The body of PRODUCE_REQUEST_V1 is the same as PRODUCE_REQUEST_V0. The version number is
     * bumped up to indicate that the client supports quota throttle time field in the response.
     */
    private static final Schema PRODUCE_REQUEST_V1 = PRODUCE_REQUEST_V0;

    /**
     * The body of PRODUCE_REQUEST_V2 is the same as PRODUCE_REQUEST_V1. The version number is
     * bumped up to indicate that message format V1 is used which has relative offset and timestamp.
     */
    private static final Schema PRODUCE_REQUEST_V2 = PRODUCE_REQUEST_V1;

    // Produce request V3 adds the transactional id which is used for authorization when attempting
    // to write
    ...
    /**
     * The body of PRODUCE_REQUEST_V4 is the same as PRODUCE_REQUEST_V3. The version number is
     * bumped up to indicate that the client supports KafkaStorageException. The
     * KafkaStorageException will be translated to NotLeaderForPartitionException in the response if
     * version <= 3
     */
    private static final Schema PRODUCE_REQUEST_V4 = PRODUCE_REQUEST_V3;

    /**
     * The body of the PRODUCE_REQUEST_V5 is the same as PRODUCE_REQUEST_V4. The version number is
     * bumped since the PRODUCE_RESPONSE_V5 includes an additional partition level field: the
     * log_start_offset.
     */
    private static final Schema PRODUCE_REQUEST_V5 = PRODUCE_REQUEST_V4;

    /**
     * The version number is bumped to indicate that on quota violation brokers send out responses
     * before throttling.
     */
    ...
    /**
     * V8 bumped up to add two new fields record_errors offset list and error_message to {@link
     * org.apache.kafka.common.requests.ProduceResponse.PartitionResponse} (See KIP-467)
     */
    ...
    public static class Builder extends AbstractRequest.Builder<ProduceRequest> {
        ...
        public static Builder forCurrentMagic(
                short acks, int timeout, Map<TopicPartition, MemoryRecords> partitionRecords) {
            return forMagic(RecordBatch.CURRENT_MAGIC_VALUE, acks, timeout, partitionRecords, null);
        }

        public static Builder forMagic(
                byte magic,
                short acks,
                int timeout,
            ...
            return new Builder(
                    minVersion, maxVersion, acks, timeout, partitionRecords, transactionalId);
        }
        ...
        public String toString() {
            ...
            bld.append("(type=ProduceRequest")
                    ...
                    .append(transactionalId != null ? transactionalId : "")
                    .append("'");
            return bld.toString();
        }
    }
    ...
    // produce request is
    // put in the purgatory (due to client throttling, it can take a while before the response is
    // sent).
    ...
    private static Map<TopicPartition, Integer> createPartitionSizes(
            Map<TopicPartition, MemoryRecords> partitionRecords) {
    ...
    public ProduceRequest(Struct struct, short version) {
        ...
        for (Object topicDataObj : struct.getArray(TOPIC_DATA_KEY_NAME)) {
            ...
            for (Object partitionResponseObj : topicData.getArray(PARTITION_DATA_KEY_NAME)) {
                ...
                MemoryRecords records =
                        (MemoryRecords) partitionResponse.getRecords(RECORD_SET_KEY_NAME);
                setFlags(records);
        ...
        transactionalId = struct.getOrElse(CommonFields.NULLABLE_TRANSACTIONAL_ID, null);
    }
    ...
    public Struct toStruct() {
        ...
        Map<String, Map<Integer, MemoryRecords>> recordsByTopic =
                CollectionUtils.groupPartitionDataByTopic(partitionRecords);
        struct.set(ACKS_KEY_NAME, acks);
        struct.set(TIMEOUT_KEY_NAME, timeout);
        struct.setIfExists(CommonFields.NULLABLE_TRANSACTIONAL_ID, transactionalId);

        List<Struct> topicDatas = new ArrayList<>(recordsByTopic.size());
        for (Map.Entry<String, Map<Integer, MemoryRecords>> topicEntry :
            ...
            for (Map.Entry<Integer, MemoryRecords> partitionEntry :
                ...
                Struct part =
                        topicData
                                .instance(PARTITION_DATA_KEY_NAME)
                                .set(CommonFields.PARTITION_ID, partitionEntry.getKey())
    ...
    public String toString(boolean verbose) {
        ...
        if (verbose)
            bld.append(",partitionSizes=")
                    .append(Utils.mkString(partitionSizes, "[", "]", "=", ","));
        else bld.append(",numPartitions=").append(partitionSizes.size());
    ...
    /**
     * Returns the partition records or throws IllegalStateException if clearPartitionRecords() has
     * been invoked.
     */
    ...
    public static void validateRecords(short version, MemoryRecords records) {
        ...
        // clients which send the wrong magic version in the wrong version of the produce request.
        // The broker
        ...
        switch (produceRequestVersion) {
            ...
            default:
                ...
                throw new IllegalArgumentException(
                        "Magic value to use for produce request version "
