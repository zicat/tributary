...
public class ProduceResponse extends AbstractResponse {
     ...
     * Possible error code:
     *
     * <p>{@link Errors#CORRUPT_MESSAGE} {@link Errors#UNKNOWN_TOPIC_OR_PARTITION} {@link
     * Errors#INVALID_TOPIC_EXCEPTION} {@link Errors#RECORD_LIST_TOO_LARGE} {@link
     * Errors#NOT_ENOUGH_REPLICAS} {@link Errors#NOT_ENOUGH_REPLICAS_AFTER_APPEND} {@link
     * Errors#INVALID_REQUIRED_ACKS} {@link Errors#TOPIC_AUTHORIZATION_FAILED} {@link
     * Errors#UNSUPPORTED_FOR_MESSAGE_FORMAT} {@link Errors#INVALID_PRODUCER_EPOCH} {@link
     * Errors#CLUSTER_AUTHORIZATION_FAILED} {@link Errors#TRANSACTIONAL_ID_AUTHORIZATION_FAILED}
    ...
    private static final Field.Int64 LOG_START_OFFSET_FIELD =
            new Field.Int64(
                    LOG_START_OFFSET_KEY_NAME,
    ...
    private static final Field.NullableStr BATCH_INDEX_ERROR_MESSAGE_FIELD =
            new Field.NullableStr(
                    BATCH_INDEX_ERROR_MESSAGE_KEY_NAME,
                    "The error message of the record that caused the batch to be dropped");
    private static final Field.NullableStr ERROR_MESSAGE_FIELD =
            new Field.NullableStr(
                    ERROR_MESSAGE_KEY_NAME,
                    "The global error message summarizing the common root cause of the records that caused the batch to be dropped");

    private static final Schema PRODUCE_RESPONSE_V0 =
            new Schema(
                    new Field(
                            RESPONSES_KEY_NAME,
                            new ArrayOf(
    ...
    /**
     * PRODUCE_RESPONSE_V2 added a timestamp field in the per partition response status. The
     * timestamp is log append time if the topic is configured to use log append time. Or it is
     * NoTimestamp when create time is used for the topic.
     */
                                                    ...
                                                    new ArrayOf(
                                                            new Schema(
                                                                    ...
                                                                    new Field(
                                                                            ...
                                                                            Type.INT64,
                                                                            "The timestamp returned by broker after appending "
    ...
    /**
     * The body of PRODUCE_RESPONSE_V4 is the same as PRODUCE_RESPONSE_V3. The version number is
    ...
    /**
     * Add in the log_start_offset field to the partition response to filter out spurious
     * OutOfOrderSequencExceptions on the client.
     */
                                                                    ...
                                                                    new Field(
                                                                            ...
                                                                            new ArrayOf(
                                                                                    new Schema(
                                                                                            new Field
                                                                                                    .Int32(
                                                                                                    BATCH_INDEX_KEY_NAME,
                                                                                                    "The batch index of the record "
    ...
    public ProduceResponse(Struct struct) {
        ...
        this.throttleTimeMs =
                struct.getOrElse(CommonFields.THROTTLE_TIME_MS, DEFAULT_THROTTLE_TIME);
    }
    ...
    protected Struct toStruct(short version) {
        ...
        for (Map.Entry<String, Map<Integer, PartitionResponse>> entry :
            ...
            for (Map.Entry<Integer, PartitionResponse> partitionEntry :
                ...
                // If producer sends ProduceRequest V3 or earlier, the client library is not
                // guaranteed to recognize the error code
                // for KafkaStorageException. In this case the client library will translate
                // KafkaStorageException to
                // UnknownServerException which is not retriable. We can ensure that producer will
                // update metadata and retry
                // by converting the KafkaStorageException to NotLeaderForPartitionException in the
                // response if ProduceRequest version <= 3
                if (errorCode == Errors.KAFKA_STORAGE_ERROR.code() && version <= 3)
                    errorCode = Errors.NOT_LEADER_FOR_PARTITION.code();
                Struct partStruct =
                        topicData
                                .instance(PARTITION_RESPONSES_KEY_NAME)
                                .set(CommonFields.PARTITION_ID, partitionEntry.getKey())
                                .set(CommonFields.ERROR_CODE, errorCode)
                    ...
                    if (!part.recordErrors.isEmpty()) {
                        ...
                        for (RecordError indexAndMessage : part.recordErrors) {
                            Struct indexAndMessageStruct =
                                    partStruct
                                            .instance(RECORD_ERRORS_KEY_NAME)
        ...
        struct.setIfExists(CommonFields.THROTTLE_TIME_MS, throttleTimeMs);
    ...
    public static final class PartitionResponse {
        ...
        public PartitionResponse(
                Errors error, long baseOffset, long logAppendTime, long logStartOffset) {
