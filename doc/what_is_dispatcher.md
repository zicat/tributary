...
3. Sink端支持对各个分发系统的对接，分发系统保证Sink端实现的可定制化。
   ...
   Sink下游主要包括HDFS、KAFKA、ES等系统。

   Sink需要屏蔽消费Channel的技术细节，同时预留接口支持对Channel数据进行格式转化并输出到特定系统中。

## 开源产品对比

开源产品中，[Apache Flume](https://flume.apache.org/)是各大公司使用较多的产品，和Apache Flume相比，我们项目的优势在于：

1. Flume对于多路分发采用的是写放大设计

   ![image](picture/flume_multi_sink.png)

   当分发数据集规模较小的场景下，写放大所带来的开销可以接受，但对于大规模的数据集处理，这样的设计基本无法满足对成本和性能的要求。

   PS：Flume对于Channel接口的定义只能做到单个Sink消费，如果改动这块设计基本是不可能完成的工作，因为很多Flume Channel的实现如Memory Channel等都没有考虑多Sink的场景。

2. Sink HDFS问题

   Flume Sink HDFS存在小文件问题，并发问题，鉴权性能问题等。  
