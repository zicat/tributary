...
public class KerberosAuthenticator implements TributaryAuthenticator {
    ...
    private final Map<String, PrivilegedExecutor> proxyCache = new HashMap<>();
    ...
    /**
     * When valid principal and keytab are provided and if authentication has not yet been done for
     * this object, this method authenticates the credentials and populates the ugi. In case of null
     * or invalid credentials IllegalArgumentException is thrown. In case of failure to
     * authenticate, SecurityException is thrown. If authentication has already happened on this
     * KerberosAuthenticator object, then this method checks to see if the current credentials
     * passed are same as the validated credentials. If not, it throws an exception as this
     * authenticator can represent only one Principal.
     *
    ...
    public synchronized void authenticate(String principal, String keytab) {
        ...
        Preconditions.checkArgument(
                ...
                "Invalid Kerberos principal: " + principal);
        Preconditions.checkArgument(
                keytab != null && !keytab.isEmpty(), "Invalid Kerberos keytab: " + keytab);
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Host lookup error resolving kerberos principal ("
                            + principal
                            + "). Exception follows.",
                    e);
        }
        ...
        final KerberosUser newUser = new KerberosUser(resolvedPrincipal, keytab);
        Preconditions.checkState(
                prevUser == null || prevUser.equals(newUser),
        ...
        try {
            ...
            LOG.warn(
                    "User unexpectedly had no active login. Continuing with " + "authentication",
                    e);
        }
        ...
        try {
            if (ugi != null) {
                if (curUser != null && curUser.getUserName().equals(ugi.getUserName())) {
                    ...
                    LOG.info("Attempting kerberos Re-login as principal ({}) ", ugi.getUserName());
            ...
            throw new SecurityException(
                    "Authentication error while attempting to "
                            + "login as kerberos principal ("
                            + resolvedPrincipal
                            + ") using "
                            + "keytab ("
                            + keytab
    ...
    private void printUGI(UserGroupInformation ugi) {
        if (ugi != null) {
            ...
            LOG.info(
                    "\n{} \nUser: {} \nAuth method: {} \nKeytab: {} \n",
                    authMethod.equals(UserGroupInformation.AuthenticationMethod.PROXY)
                            ? "Proxy as: "
                            : "Logged as: ",
                    ugi.getUserName(),
                    authMethod,
    ...
    /**
     * startCredentialRefresher should be used only for long running methods like Thrift source. For
     * all privileged methods that use a UGI, the credentials are checked automatically and
     * refreshed before the privileged method is executed in the UGIExecutor
     */
    @Override
