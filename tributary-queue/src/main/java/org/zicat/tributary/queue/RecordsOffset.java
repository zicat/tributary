 ...
 * RecordsOffset.
 *
 * <p>Each block in {@link LogQueue} has a unique RecordsOffset.
 *
 * <p>RecordsOffset can be stored and fetch by {@link LogQueue} using group id.
 *
 * <p>RecordsOffset is composed of segmentId(long) and offset(long). It is important to control the
 * size of segment. If the size is too small will cause {@link
 * org.zicat.tributary.queue.file.FileLogQueue} put records performance poor.If the size is to large
 * will cause {@link org.zicat.tributary.queue.file.FileLogQueue} clear expired segment not timely.
 */
